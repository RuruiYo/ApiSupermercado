...
namespace SupermercadoAPI.Services
{
    public class ProductoService
    {
        ...
        public List<ProductoResponseDto> GetCatalogo(string? nombre, int? categoriaId, decimal? precioMin, decimal? precioMax)
        {
            ...
            return query.Select(p => ToDto(p)).ToList();
        }
        ...
        public (bool Exito, string Mensaje, ProductoResponseDto? Datos) GetPorId(int id)
        {
            ...
            if (p == null)
                return (false, $"No se encontró el producto con ID {id}.", null);
        ...
        public (bool Exito, string Mensaje, int ID) Crear(ProductoCreateDto dto)
        {
            if (_context.Productos.Any(p => p.SKU_CodigoInterno == dto.SKU_CodigoInterno))
                return (false, "Ya existe un producto con ese SKU.", 0);

            if (!_context.Categorias.Any(c => c.ID_Categoria == dto.ID_Categoria))
                return (false, "La categoría especificada no existe.", 0);
        ...
        public (bool Exito, string Mensaje) Actualizar(int id, ProductoUpdateDto dto)
        {
            ...
            if (producto == null)
                return (false, $"No se encontró el producto con ID {id}.");
            ...
            producto.UbicacionBodega = dto.UbicacionBodega;
        ...
        // Método privado reutilizable para mapear a DTO
        private static ProductoResponseDto ToDto(Producto p) => new ProductoResponseDto
        {
            ID_Producto = p.ID_Producto,
            SKU_CodigoInterno = p.SKU_CodigoInterno,
            NombreProducto = p.NombreProducto,
            Descripcion = p.Descripcion,
            UbicacionBodega = p.UbicacionBodega,
